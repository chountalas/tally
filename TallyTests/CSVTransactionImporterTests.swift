...
final class CSVTransactionImporterTests: XCTestCase {
    ...
    func testDraftDetectionPenalizesMetadataRowsBeforeGenericHeaders() throws {
        let importer = CSVTransactionImporter()
        let csv = """
        Export generated by Example Bank
        Account,Visa
        When,Spent,Who
        2025-01-01,-15.49,Netflix
        2025-02-01,-10.99,Spotify
        """

        let draft = try importer.makeDraft(fileName: "generic-header-preamble.csv", csvText: csv)
        let transactions = try importer.materializeTransactions(from: draft, mapping: draft.suggestedMapping)

        XCTAssertEqual(draft.headers, ["When", "Spent", "Who"])
        XCTAssertEqual(draft.suggestedMapping.dateColumn, "When")
